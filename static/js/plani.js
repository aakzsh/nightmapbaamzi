...
function get_user_obs()
{
   ...
   if ( parseInt(lon.innerText) >= -180 && parseInt(lon.innerText) < 360 )
      now.setLonDegrees( parseInt(lon.innerText) );
      
   if ( parseInt(lat.innerText) >= -90 && parseInt(lat.innerText) <= 90 )
      now.setLatDegrees( parseInt(lat.innerText) );

   ck_starlabels = slab.checked;
   ck_conlabels = clab.checked;
   ck_dsos = idso.checked;
   ck_conlines = clin.checked;
   console.log(parseInt(lat.innerText), parseInt(lon.innerText))
   // console.log( "slab " + ck_starlabels + " dsos " + ck_dsos );
   set_user_obs();

   console.log('trynna save from get')
   refresh();
...
function canvasApp()
{
   ...
   init_planets( planet );
   now = new Observer();
   console.log("hitting set user")
   set_user_obs();
   refresh();
   console.log("hitting ref")
}
